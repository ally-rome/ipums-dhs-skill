...
- `--filter`: filter rows before computing. Supports equality (`HHLINENO=1`) and inequality operators (`KIDCURAGEMO>=12`, `KIDCURAGEMO<=23`). Use single quotes around filters with `>=` or `<=` to prevent shell redirection.
